...
interface FlagExplanationSystemProps {
  flags: FlagEvent[];
  className?: string;
}
...
export const FlagExplanationSystem: React.FC<FlagExplanationSystemProps> = ({
  flags,
  className = ''
}) => {
  ...
  const flagExplanations: Record<string, FlagExplanation> = {
    EYES_OFF: {
      type: 'EYES_OFF',
      title: 'Eyes Off Screen',
      description: 'The system detected that your gaze was directed away from the quiz content for an extended period.',
      commonCauses: [
    ...
    HEAD_POSE: {
      ...
      description: 'Your head position moved outside the acceptable range, which may indicate looking at unauthorized materials.',
      commonCauses: [
    ...
    TAB_BLUR: {
      ...
      description: 'The quiz tab lost focus, indicating you may have switched to another application or browser tab.',
      commonCauses: [
    ...
    SECOND_FACE: {
      ...
      description: 'The system detected another person in the camera view, which violates quiz integrity policies.',
      commonCauses: [
    ...
    DEVICE_OBJECT: {
      ...
      description: 'The system detected what appears to be an electronic device (phone, tablet, etc.) in your workspace.',
      commonCauses: [
    ...
    SHADOW_ANOMALY: {
      ...
      description: 'Significant changes in lighting or shadows were detected, which may indicate environmental manipulation.',
      commonCauses: [
    ...
    FACE_MISSING: {
      ...
      description: 'The system temporarily lost track of your face, which may indicate you left the camera view.',
      commonCauses: [
    ...
    DOWN_GLANCE: {
      ...
      description: 'The system detected a pattern of frequent downward glances, which may indicate reading notes.',
      commonCauses: [
  ...
  const groupedFlags = flags.reduce((acc, flag) => {
    ...
    return acc;
  }, {} as Record<string, FlagEvent[]>);

  const toggleExplanation = (flagType: string) => {
    ...
    return (
      <div className={`bg-green-50 border border-green-200 rounded-lg p-6 text-center ${className}`}>
        ...
        <p className="text-green-700">
          Great job! No academic integrity concerns were detected during your quiz.
        </p>
      </div>
    );
  }
    ...
    <div className={`bg-white rounded-lg shadow-sm border border-gray-200 ${className}`}>
      <div className="p-6 border-b border-gray-200">
        ...
        <p className="text-gray-600">
          Understanding your academic integrity alerts and how to prevent them in the future.
        </p>
      </div>
      ...
      <div className="p-6">
        ...
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          <div className="text-blue-800 text-sm">
            <p className="mb-2">
              Your quiz session generated <strong>{flags.length}</strong> integrity alert{flags.length !== 1 ? 's' : ''} 
              across <strong>{Object.keys(groupedFlags).length}</strong> different categor{Object.keys(groupedFlags).length !== 1 ? 'ies' : 'y'}.
            </p>
            <p>
              <strong>Hard alerts:</strong> {flags.filter(f => f.severity === 'hard').length} | 
              <strong> Soft alerts:</strong> {flags.filter(f => f.severity === 'soft').length}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {Object.entries(groupedFlags).map(([flagType, flagList]) => {
            ...
            const hardFlags = flagList.filter(f => f.severity === 'hard').length;
            const softFlags = flagList.filter(f => f.severity === 'soft').length;

            return (
              <div key={flagType} className="border border-gray-200 rounded-lg">
                <button
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {flagList.length} occurrence{flagList.length !== 1 ? 's' : ''}
                          {hardFlags > 0 && (
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 border-t border-gray-100">
                    <div className="mt-4 space-y-4">
                      {/* Description */}
                      <div>
                        <h6 className="font-medium text-gray-900 mb-2">What happened?</h6>
                        <p className="text-gray-700 text-sm">{explanation.description}</p>
                      </div>

                      {/* Occurrences */}
                      <div>
                        <h6 className="font-medium text-gray-900 mb-2">
                          When did this occur? ({flagList.length} time{flagList.length !== 1 ? 's' : ''})
                        </h6>
                        <div className="space-y-2">
                          {flagList.slice(0, 5).map((flag, index) => (
                            <div key={flag.id} className="text-sm text-gray-600 bg-gray-50 p-2 rounded">
                              <div className="flex items-center justify-between">
                                <span>
                                  {new Date(flag.timestamp).toLocaleTimeString()}
                                  {flag.questionId && ` (Question ${flag.questionId})`}
                                </span>
                          ...
                          {flagList.length > 5 && (
                            <div className="text-sm text-gray-500 text-center">
                              ... and {flagList.length - 5} more occurrence{flagList.length - 5 !== 1 ? 's' : ''}
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      {/* Common causes */}
                      <div>
                        <h6 className="font-medium text-gray-900 mb-2">Common causes:</h6>
                      ...
                      {/* Prevention */}
                      <div>
                        <h6 className="font-medium text-gray-900 mb-2">How to prevent this:</h6>
                        <ul className="text-sm text-gray-700 space-y-1">
